...
- When the consult agent asks the operator a question with [`ask_user`](/tools/ask-user), the prompt is posted to the conversation that requested the call, for example the Telegram chat that placed it, and stays answerable from the Control UI, TUI, and native apps. A call with no deliverable requesting conversation keeps the question in those surfaces only.
