...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: OpenClaw captures participant audio directly from the browser's remote playback and injects assistant audio into the virtual microphone through the output command. The input command verifies that injected audio reaches the virtual microphone. `agent` mode uses realtime transcription plus regular TTS; `bidi` mode uses the realtime voice provider. The default path is 24 kHz PCM16 with `chrome.audioBufferBytes: 4096`; 8 kHz G.711 mu-law remains available for legacy command pairs.
...
Local Chrome and paired Chrome nodes use the same isolated browser capture and native injection path. If browser playback cannot be captured, the managed bridge fails clearly instead of sending mixed loopback audio to the provider. A room with no participant audio yet is idle, not a capture failure.
