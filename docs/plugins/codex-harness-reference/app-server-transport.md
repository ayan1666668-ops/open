...
| `loopDetectionPreToolUseRelay`   | `true`                                                 | Enables the Codex `PreToolUse` relay for loop detection when OpenClaw loop detection is enabled. OpenClaw installs no `PreToolUse` relay when no before-tool policy, enabled loop detector, or sandbox native-process admission has local work. Set `false` to disable the loop-detection relay; before-tool policy and sandbox process admission still install their required relay.                                              |
| `nativeHookRelay`                | `{ enabled: true }`                                    | Native hook relay controls. `enabled: false` omits OpenClaw's hook configuration and leaves native hook layers unchanged when the effective approval policy is `never`, no before-tool policy is active, and no sandbox native-process admission is required. Otherwise it narrows to the required `pre_tool_use` relay with a warning.                                                                                            |
...
`never`, no before-tool policy is active, and no sandbox native-process admission
is required. Otherwise, it keeps the required `pre_tool_use` relay and logs a
warning. Sandbox process admission retains only that required event when optional
relays are disabled. Required `pre_tool_use` callbacks can still apply configured
loop detection. Native approval routing remains active.
