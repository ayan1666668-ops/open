...
| `chrome.audioInputCommand`        | generated native command                 | Captures the virtual microphone for assistant-output verification; participant input comes from isolated browser playback. Uses SoX/CoreAudio on macOS or `parec` on Linux.                            |
