...
falls back to built-in LLM summarization rather than cancelling. Cancellation is
reserved for a failed built-in generation, an unresolvable summarization model,
and missing credentials, which keep the original history. Caller-initiated aborts
and transport timeouts are re-thrown, so explicit cancellation is always
respected.
