...
Restore recreates `openclaw/<name>` at the original pre-snapshot commit, reusing its clean source template when available. Git applies the saved differences as unstaged modifications and untracked files without replacing the template with snapshot contents. Disk admission includes space for changed and added files alongside clone metadata; an ordinary checkout also includes the original source files. If the snapshot changes `.gitattributes`, Git rematerializes all snapshot files so unchanged blobs receive the correct line endings and filters, with a full-copy space allowance. This keeps the synthetic snapshot commit out of branch history. The snapshot ref remains recorded as provenance.
