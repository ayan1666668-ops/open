...
The resulting session row references the managed worktree by its stable ID, and every agent run in that session uses its checkout. Explicitly naming the same worktree from another session makes the checkout a shared resource; it is not reassigned to either session. When the workspace is a repository subdirectory, the worktree is anchored at the repository root and the session runs from the matching subdirectory inside it. Session worktree creation uses the method's `operator.write` scope. Repository checkout/ref hooks and filesystem monitors are always disabled. The `.openclaw/worktree-setup.sh` step runs only for an `operator.admin` caller; retries evaluate the current caller's scope rather than retaining the original caller's permission. `.worktreeinclude` provisioning still applies to every caller. Deleting a session attempts to snapshot and remove a worktree that is still assigned only to that session. Detaching from a shared worktree leaves its checkout live. Hourly cleanup snapshots session worktrees after 7 idle days, treating recent session activity as worktree activity. Removed worktrees remain restorable from their snapshots as described below.

Archiving a session commits its archive state, then snapshots and removes a checkout assigned only to that session while preserving the conversation and worktree binding. A shared checkout stays live for the other session references and is eventually handled by idle cleanup. A failed metadata write leaves the checkout untouched. If exclusive-checkout cleanup cannot finish safely, the session remains archived, its files stay preserved, and the error explains that cleanup is pending. Repeating archive retries cleanup; startup and hourly garbage collection also retry any checkout that remains. Automatic session archival uses the same metadata-first ordering.
