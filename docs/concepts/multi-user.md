...
New mentions also appear as a compact notification at the top right of each connected Control UI tab, except a tab already showing that session. The notification includes the sender, session name, message excerpt, and **View session** action. It disappears after five seconds, pausing while hovered or keyboard-focused. Closing the notification does not dismiss the Inbox entry or hide notifications in other tabs. Reconnecting seeds the retained Inbox without replaying its old entries as in-app notifications.
