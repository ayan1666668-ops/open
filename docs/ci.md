...
- Hosted `check-lint` restores and saves the same content-keyed extension package-boundary archive as the dedicated boundary lane before rebuilding any missing declarations; Blacksmith keeps its read-only sticky-disk fast path. The Control UI and UI E2E jobs share a Linux Playwright Chromium archive keyed by the exact pinned Playwright version. In hybrid mode, the protected warmer adds one `ubuntu-24.04` row so hosted Linux consumers can restore from GitHub's cache backend instead of depending on Blacksmith's separate cache. Every cache-enabled Linux setup uses the publisher's workspace-local store path because Actions includes the path in cache compatibility. The hosted Linux row installs dependencies and publishes only the pnpm store immediately after setup; it runs no build or test warming. A standard hosted `macos-15` row does the same for macOS. Both use the existing OS, architecture, Node-version, package, and lockfile key, while ordinary CI remains restore-only.
...
The protected cache warmer normally has two platform rows: the existing Linux workload and one hosted macOS pnpm-store publisher. Hybrid mode adds one hosted Linux pnpm-store publisher because Blacksmith and GitHub-hosted runners use separate cache backends. Its per-ref concurrency and pending-run coalescing are unchanged. Each admitted hybrid warmer run adds two hosted jobs and no new Blacksmith registrations; pull-request CI adds no writers or jobs. Native producer and consumer measurements must include cache transfer, extraction, installation, and archive size before claiming a setup-time saving.
