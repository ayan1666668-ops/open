...
Typecheck rows restore incremental compiler state published by the protected cache warmer on both Linux cache backends. Every hit still runs project-mode checking, including discovery of new files. Restore logs distinguish exact source hits, compatible seeds, and misses.
