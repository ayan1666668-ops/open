...
fast/check lanes remain capped at 12 in both modes; measured Windows rows are capped at five
...
Compact descriptor counts and predicted maxima vary with the committed measurements; the serial TUI PTY dist descriptor keeps its indivisible measured wall. In `github` mode compact jobs run hosted; hybrid attempt 1 uses each row's 4-vCPU or 8-vCPU Blacksmith label, while hybrid retries use hosted runners. Control UI E2E expands from the unchanged all-Blacksmith shape of three Vitest shards plus one browser-extension shard to thirteen Vitest shards plus one browser-extension shard for the `github` and `hybrid` planner profiles. QA Smoke similarly expands from four parts to six. Hybrid attempt 1 runs those expanded matrices on Blacksmith; `github` mode and hybrid retries use hosted runners. QA's planner reserves the final part's observed roughly two-minute Matrix rider before greedily assigning primary scenarios, keeping that separate run from becoming the tail. Windows uses up to five measured rows of disjoint, project-aligned explicit test files. This partitions the complete Windows-specific inventory without applying Vitest `--shard` to project-local single-file selections, which Vitest rejects. Blacksmith rows run on `blacksmith-16vcpu-windows-2025`; projects remain serial within each row while Vitest may use up to four workers. Hosted fallback rows keep serial projects and one Vitest worker. The Windows cron process-identity proof uses the existing runtime prerequisite owner before starting its real Gateway; it does not require the broader E2E declaration and private-QA builds. Worker-artifact cases use independent per-test fixture lifetimes with at most two concurrent cases; the case that observes process-wide preparation logs remains sequential. The planner assigns every selected file exactly once, keeps runtime consumers with their prerequisite owner, and starts the largest selected project first; generated worker artifacts and per-project caches retain their existing ownership. The requested label does not verify available CPUs or RAM, so native proof must cover concurrent fixture memory and cleanup. A hybrid retry reruns the measured rows on hosted `windows-2025`, slower but bounded. Expect slower individual builds on standard 4-core hosted runners. Blacksmith's runner-registration budget is irrelevant for hosted jobs, but GitHub-hosted concurrency limits apply.
