...
whitespace-only channel values fail with `--channel must not be blank` before plugin setup or
lookup, so an unset shell variable cannot list a directory you did not name.
