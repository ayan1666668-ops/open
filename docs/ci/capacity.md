...
Core storage/state stripes also have a 64-file admission ceiling. In [run 35477045216](https://github.com/openclaw/openclaw/actions/runs/35477045216), a 203-file serial stripe continued passing tests until the one-hour job deadline; its 196-file sibling completed in 2,867 test seconds. The compact split owner bounds each prerequisite-preserving stripe independently of older timing estimates, and the existing distinct-family rule prevents reassembly in one job. Every file remains covered once, with fork isolation, serial file execution, worker limits, deadlines, and the 80-row compact cap unchanged. This is a workload bound, not a measured new runtime; hosted CI must establish the resulting duration.
