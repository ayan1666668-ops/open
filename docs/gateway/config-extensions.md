...
- `plugins.entries.<id>.subagent.allowRun`: consents to that exact plugin's manifest-declared `runtimeCapabilities: ["subagent.run"]`. Both declaration and consent are required. This grants only the host-owned subagent run facade, not general Gateway or operator access.
