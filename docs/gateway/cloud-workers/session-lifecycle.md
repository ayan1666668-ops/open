...
Workspace quiescence retries slow process probes within one 30-second budget. The recovery watchdog keeps unfinished processes across exhausted passes so slow probes cannot repeatedly resume the same workers and starve the rest. Probe retries share their pass deadline; only an exhausted pass opens another budget for its remaining work. Each probe starts with a two-second allowance and gets more time after a timeout; exhausted probes report the budget and suggest checking host load and `ps` availability. Failed reconciliation retains the recoverable workspace result and reports the reason through the normal recovery flow.
