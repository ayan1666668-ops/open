...
Workspace quiescence retries slow process probes within one 30-second budget. Each probe starts with a two-second allowance and gets more time after a timeout; exhausted probes report the budget and suggest checking host load and `ps` availability. Failed reconciliation retains the recoverable workspace result and reports the reason through the normal recovery flow.
