...
- `gateway.workspacePreviewMaxBytes`: shared byte cap for inline Control UI file preview content served by `sessions.files.get`, `sessions.files.set`, and `agents.workspace.get` (logs, data exports, and build artifacts routinely exceed the default). Files above the cap come back as metadata-only entries; oversize text previews report `session_file_too_large` with the effective cap. Default: `262144` (256 KiB). Accepts `1`–`4194304` (4 MiB); the cap is on raw file bytes and the bound keeps the worst-case JSON-escaped or base64-encoded frame under the 25 MiB Gateway WebSocket payload limit. Changes hot-apply per request; remote repository workspace previews (`repository.inspect`) keep the default cap and report it.
