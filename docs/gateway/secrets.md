...
> **Note**: The `default` provider is a built-in alias for `env` and `store` sources and does not need to be explicitly defined. For `file` and `exec` sources, `default` is not a built-in alias — you must declare a matching `secrets.providers.default` entry (or use a custom alias) or resolution will fail with `SECRET_PROVIDER_NOT_CONFIGURED`.
