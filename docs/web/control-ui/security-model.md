...
- HTTPS transcript images render in Chat image galleries and Activity previews. The browser contacts the image host directly, disclosing its network address; thumbnails and the expanded image viewer send no page referrer. Markdown document previews retain their click-to-open treatment for remote images.
