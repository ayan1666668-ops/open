...
**Archived** hides active sessions even when their conversation remains open. In **Active**, a directly opened archived session can retain its selected row. When you archive a visible session, its row shows **Archiving…** until the Gateway finishes stopping its work and confirms the archive. Repeated archive actions stay disabled while that request is pending. If it fails, the row remains visible and the error explains what prevented archiving. Once confirmed, archive and Undo results remain reflected in the session lists even if the follow-up refresh fails. The refresh error is shown separately; it does not undo a successful archive or restore.
