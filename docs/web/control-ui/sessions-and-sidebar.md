...
The **Filter & sort** menu starts with **Filters** and **View**, each showing a
summary of your current choices. **Filters** controls which sessions appear,
including status, owner, automation, and system sessions. **View** controls
grouping, sorting, message previews, and empty groups. **Session sources** opens
its Settings destination separately. Choose **Back** or press Escape to return
to the menu; Escape at the root closes it. Choices take effect immediately and
keep the current page open. In **Show all agents** mode, View omits grouping and
empty-group controls because the sidebar always groups by agent.
