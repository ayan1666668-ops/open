...
The chat transcript uses a centered readable frame aligned with the composer. Assistant and tool output stay left-aligned while your own messages stay right-aligned inside that frame. In multi-user sessions (for example a group chat relayed from a channel plugin), messages from other attributed participants render left-aligned with the author's avatar, name, and a stable per-identity color, so only the signed-in viewer's messages read as "mine". When two or more attributed participants are present, assistant replies carry one "Replying to" line with the participant’s avatar, name, and an excerpt of the answered prompt, separated by contrast rather than punctuation. On desktop, the agent avatar stays at the top of attributed reply groups, with a short curved connector running from behind it to that line. On narrow screens, agent and participant avatars beside messages stay hidden to preserve content width; a small reply arrow introduces the attribution instead of the connector, while the quoted author’s avatar remains visible. These outside headers omit the excerpt on narrow screens; select the quoted author to navigate to the original. Your own reply header stays inside the bubble and keeps its excerpt at every screen size. The excerpt uses the first useful line without Markdown, with CSS truncation in a compact single line; attachment-only prompts show a file icon and the attachment name without quotation marks. Select the excerpt to scroll to and briefly highlight a loaded original. Unloaded originals keep the available preview as plain text. When no excerpt is available, the line shows "Original message unavailable" without quotation marks or a link. System entries such as local slash-command output render as centered notice rows without an avatar.
