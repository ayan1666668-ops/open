...
The Control UI binds each [selected person](/concepts/multi-user#mentioning-people) to the submitted message text. `chat.send`, the initial message on `sessions.create`, and `sessions.send` accept an optional `mentions` array of `{ profileId, start, end }` person annotations or explicit `{ kind: "everyone", start, end }` broadcast annotations. There are at most ten annotations. `start` is inclusive and `end` is exclusive, measured in UTF-16 code units. The Gateway validates their text ranges and recipients before accepting the input. The broadcast span must contain exactly `@everyone`. Its server-selected audience is bounded to 1,000 eligible human profiles and excludes the sender. Oversized audiences are rejected rather than truncated. Plain `@name` or `@everyone` text and agent output do not create human mentions. Copying or quoting text does not copy its recipient selections.
...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `users.mentionable` | Search with `{ sessionKey, agentId?, query? }`, or use `{ agentId, visibility?, query? }` before creating a session. Returns bounded `{ users, truncated, everyone? }` results with profile id, display name, optional avatar URL, and online status. The optional `everyone` choice includes `recipientCount` for the current eligible audience. |
