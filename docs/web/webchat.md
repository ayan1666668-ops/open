...
- In the Control UI, assistant images and attachments appear in message order, between their surrounding paragraphs. Adjacent images within one reply appear side by side when their normal previews fit, and wrap to the next line when they do not. There is no horizontal scroller or cropping. Text and other attachments separate image sets, and separately delivered messages stay separate, so before/after labels and conversation order are preserved.
