...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ...
    - `streaming.progress.previewWithHooks` (default: `false`) is an explicit waiver for the progress draft. Plugins that register `reply_payload_sending` or `message_sending` hooks normally suppress every provider preview, because a draft is visible before those hooks run. With this option the progress draft stays, and its status headline (from assistant preambles), plan milestones, approval requests and tool lines reach Telegram without passing through those hooks; every durable message, including the final answer, still does. It applies only in `progress` mode without `progress.commentary` and without streamed reasoning; `partial` and `block` previews stay suppressed.
