...
- Pass `messageId=<postId>` (CLI `--message-id`) to read one post. OpenClaw locates it through the target channel's history rather than a server-wide post lookup, so a post from another channel fails as not found.
