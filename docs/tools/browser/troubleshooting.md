...
- After launching a local managed browser, readiness probes allow up to 1.5 seconds per HTTP request and 2 seconds per WebSocket stage to tolerate Gateway scheduling delays. The readiness retry window defaults to eight seconds (`browser.localCdpReadyTimeoutMs`); probes near its end use shorter timeouts.
