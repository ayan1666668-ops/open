...
<ParamField path="inheritParentGroup" type="boolean" default="false">
  Copy the parent's current sidebar group once when creating a visible session. Requires `visible: true` and an omitted `group`. An explicit `group` wins, including empty and whitespace-only values that keep the child ungrouped. Later parent and child group changes remain independent.
</ParamField>
