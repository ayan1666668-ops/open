...
Layer 3 extends the generic contract with an optional exact candidate manifest:
...
When present, OpenClaw validates the complete candidate/source/recipe/policy
manifest, computes a stable candidate identity, and serializes both manifest and
identity into the prepared launch bytes before the existing replay fingerprint is
computed.

A conflicting `handoff.candidateDigest` is rejected.

This proves which exact candidate bytes were selected for verification. It does
not prove that verification succeeded, that a verifier is independent, or that an
