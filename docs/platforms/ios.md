...
- While a chat response is active, you can compose the next message and keep using **Stop response**. The composer explains that sending becomes available when the response finishes. Stopping the response preserves your draft; mid-run queueing and steering are not available in the native composer.
