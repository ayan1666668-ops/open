...
Talk opens a dedicated conversation page. During an admitted call, the composer
waveform returns to that same conversation without starting or ending audio.
**Go to chat** returns to the original agent/session; Android Back changes only
the visible page. The conversation's **End** control is separate from the softly
tinted chat **Stop** button, which aborts the agent run. See [Talk client UI](/nodes/talk/client-ui)
for photo controls, activity signals, and background-call verification limits.
...
- Send: `chat.send`. You can send text or staged attachments while the agent is working, including during an admitted Chat call. A new draft brings back **Send**; clearing it restores **Stop**. The Gateway applies the existing [queue mode](/concepts/queue), so steering does not require stopping the current run. Sending remains disabled while another submission, attachment staging, dictation, or voice-note capture owns the draft.
