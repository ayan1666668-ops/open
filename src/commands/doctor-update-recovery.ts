...
} from "./doctor-update-recovery-resolution.js";
export { resolveCompletedDoctorUpdateRecovery } from "./doctor-update-recovery-resolution.js";
...
export async function prepareDoctorUpdateRecovery(options: DoctorOptions = {}): Promise<void> {
  ...
  if (supplied !== undefined) {
    // Verification already binds the selected state/config paths. Bind this exact
    // capture to its still-active driver run too, before any Doctor mutation.
    // The captured install root can differ from the fresh Doctor root during an
    // install-kind transition; the admitted run's digest authenticates it.
    const inheritedRunId = process.env[UPDATE_RUN_ID_ENV]?.trim();
    const { getUpdateRun } = await import("../infra/update-run-ledger.js");
    scope.assertRecoveryClaim = () => {
      const run = inheritedRunId ? getUpdateRun(inheritedRunId) : undefined;
      if (
        !run ||
        run.status !== "running" ||
        capturedManifest.runId !== inheritedRunId ||
        run.origin.updateRecoveryCapture?.manifestSha256 !== reference.manifestSha256
      ) {
        throw new Error("Doctor recovery capture does not belong to its active update run.");
      }
    };
  }
  assertDoctorRecoveryCurrent(scope);
