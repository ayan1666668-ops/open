...
// After whitespace: `name=value` or a supported boolean name, in any order
// (quoted and unquoted values are mutually exclusive so `<a !=` + `"" !=`
// cannot explode). `and` is not a supported boolean, so
// `attempts<max and backoffMs>0` stays prose.
const HTML_TAG_RE =
  /<\/?[a-z][a-z0-9_.:-]*(?:\/[^>]*|(?:\s+(?:[^\s"'>=/]+\s*=\s*(?:"[^"]*"|'[^']*'|[^\s"'>=]+)|(?:checked|disabled|hidden|readonly|required|selected|multiple|open)(?=[\s/>])))+)?\s*\/?>/gi;
