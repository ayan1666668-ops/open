...
export async function migrateLegacyExecutionSelections(params: {
  ...
  return withAgentDatabaseMaintenanceLease(
    ...
    async (maintenance) => {
      ...
      const assertSharedCurrent = await backUpMigrationDatabase(
        sharedPath,
      ...
      try {
        ...
        const convert = (
          ...
          if (!binding) throw new Error("Session binding is invalid; original selection retained.");
          ...
          const acp: LegacyAcpExecutionSelection | undefined = source
            ...
            : undefined;
          return {
            ...migrateSessionExecutionSelection({ entry, acp, classifyExecutor, defaultProvider }),
            source,
          };
        };
        const migrateRows = (
