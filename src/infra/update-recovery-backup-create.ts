...
async function inspectUpdateRecoveryBackup(params: CaptureParams) {
  ...
  const visit = async (pathname: string): Promise<void> => {
    ...
    if (before.isSymbolicLink()) {
      ...
      if (sqliteLink || rawFiles.has(pathname) || declaredKinds.get(pathname) === "directory") {
        ...
        if (sqliteLink) {
          ...
          rawFiles.add(target);
        } else if (declaredKinds.get(pathname) === "file") {
          if (!(await fs.stat(target)).isFile()) {
            throw new Error(`Declared recovery file symlink does not target a file: ${pathname}`);
          }
          declaredKinds.set(target, "file");
          rawFiles.add(target);
        } else {
