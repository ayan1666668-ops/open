...
import { completeUpdateCommandResult } from "./update-command-terminal-publication.js";
...
const definitionRecoverySchema = z.strictObject({
  backup: GatewayServiceDefinitionBackupReceiptSchema.optional(),
...
async function recoverMigratedUpdateInParent(
  ...
  assertCurrent();
  await printResult(completed, params.opts, { nextAction });
...
export async function continueMigratedUpdateInFreshProcess(
  ...
  try {
    ...
    if (run.executorFence || run.completionOwner || params.updateRecoveryBackup) {
      assertCurrent();
    ...
    const recoveryEvidence = definitionRecoverySchema.safeParse(response.definitionRecovery);
    definitionRecovery = recoveryEvidence.success ? recoveryEvidence.data : { unverified: true };
