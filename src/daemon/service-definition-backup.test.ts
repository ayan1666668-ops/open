...
describe("service definition backup receipts", () => {
  ...
  it.each(["candidate", "original"])(
    ...
    async (bytes) => {
      ...
      // Allocate while the original still exists, so its inode cannot be recycled
      // into the replacement when publication and replacement share a clock tick.
      await fs.copyFile(f.sourcePath, replacement);
      ...
      vi.spyOn(f.capture.hooks, "fileWritten").mockImplementationOnce(async (source, contents) => {
        await fs.writeFile(
          replacement,
          bytes === "original" ? f.original : await fs.readFile(source),
        );
