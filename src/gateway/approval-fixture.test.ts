...
describe("approval fixture request ownership", () => {
  ...
  it("joins a late body before reset-owner cleanup releases its database", async (testContext) => {
    let joined = false;
    ...
    try {
      ...
      expect(database.db.isOpen).toBe(true);
      const reset = vi.fn();
      cleanup = cleanupTestApprovalFixtures(testContext).then(reset);
      void cleanup.catch(() => {});
      expect(fixture.cleanup()).toBe(fixture.cleanup());
