...
export async function closeAcpRuntimeForSession(params: {
  ...
  try {
    ...
    if (params.reason === "session-reset") {
      const selection = getCommittedSessionExecutionSelection(
      ...
      // Lifecycle metadata alone cannot authorize a new app session.
...
export function buildPendingAcpMeta(base: SessionAcpLifecycle, now: number): SessionAcpLifecycle {
  ...
  return {
    ...
    identity: {
      ...
      source: currentIdentity?.source ?? "ensure",
      lastUpdatedAt: now,
    },
    mode: base.mode,
