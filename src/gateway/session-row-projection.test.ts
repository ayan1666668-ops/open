  ...
  await withOpenClawTestState({ scenario: "minimal" }, async () => {
    ...
    replaceSessionEntrySync(
      { agentId: "main", sessionKey: key },
      {
        sessionId: "catalog",
        updatedAt: 1,
        executionSelection: acceptedModelSelection("unit-test", "fixture"),
      },
    );
    const projection = await createSessionRowProjection({
      cfg,
    ...
    for (const agentId of ["main", "work"]) {
      replaceSessionEntrySync(
        { agentId, sessionKey: "global" },
        {
          ...
          updatedAt: 1,
          executionSelection: acceptedModelSelection("unit-test", `${agentId}-model`),
        },
      );
    }
    const key = "agent:main:child";
    ...
    try {
      ...
      replaceSessionEntrySync(
        { agentId: "work", sessionKey: "global" },
        {
          ...
          updatedAt: 3,
          executionSelection: acceptedModelSelection("unit-test", "updated-work-model"),
        },
      );
      await projection.ensureMaterialized();
      expect(projection.snapshot({ agentId: "main", key }).row).toMatchObject({
...
it("inherits a raw sentinel parent from its physical store and refreshes its dependents", async () => {
  await withOpenClawTestState({ scenario: "minimal" }, async (state) => {
    ...
    const parent = {
      sessionId: "work-parent",
      updatedAt: 1,
      executionSelection: acceptedModelSelection("unit-test", "before"),
    };
    ...
    try {
      ...
      replaceSessionEntrySync(
        ...
        { ...parent, executionSelection: acceptedModelSelection("unit-test", "after") },
      );
      await projection.ensureMaterialized();
