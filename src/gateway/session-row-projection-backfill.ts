...
export function createSessionRowProjectionBackfill(params: {
  ...
  async function drain() {
    for (;;) {
      ...
      try {
        const source = row.materialized?.source;
        const fields = await backfillSessionRowTranscriptFields({
          ...
          shouldCommit,
          model:
            source?.selectedModel.provider && source.selectedModel.model
              ? {
                  selectedProvider: source.selectedModel.provider,
                  selectedModel: source.selectedModel.model,
                  config: source.cfg,
                }
              : undefined,
        });
