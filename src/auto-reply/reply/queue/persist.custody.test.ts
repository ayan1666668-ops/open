...
describe("followup queue durable custody", () => {
  ...
  it("rejects admission for a key whose row bulk restore could not decode", () => {
    ...
    expect(listUnreadableFollowupQueueKeys()).toContain(corruptKey);
    expect(followupQueueEntryContainsPrompt(corruptKey, "fresh work")).toBe(false);
  });
  ...
  it("keeps operator-authority-bound turns out of the snapshot entirely", () => {
    const queue = getFollowupQueue(TEST_KEY, SETTINGS);
    ...
    expect(followupQueueEntryContainsPrompt(TEST_KEY, "operator-authority turn")).toBe(false);
    expect(followupQueueEntryContainsPrompt(TEST_KEY, "operator-1")).toBe(false);
    expect(followupQueueEntryContainsPrompt(TEST_KEY, "authority-free turn")).toBe(true);
  ...
  it("keeps a restart-retired row for startup recovery instead of replaying it here", () => {
    const queue = getFollowupQueue(TEST_KEY, FOLLOWUP_SETTINGS);
    ...
    expect(followupQueueEntryContainsPrompt(TEST_KEY, "retired before restart")).toBe(true);
    expect(FOLLOWUP_QUEUES.get(TEST_KEY)).toBeUndefined();

    enqueueFollowupRun(TEST_KEY, makeFollowupRun("after retirement"), FOLLOWUP_SETTINGS);
    ...
    persistFollowupQueuesOrThrow();
    expect(followupQueueEntryContainsPrompt(TEST_KEY, "retired before restart")).toBe(true);
    expect(followupQueueEntryContainsPrompt(TEST_KEY, "after retirement")).toBe(false);
  });
});
