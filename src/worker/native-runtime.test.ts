...
import type { Context } from "@openclaw/ai";
...
describe("native runtime startup authority", () => {
  ...
  it("executes real HTTP in an isolated native process using startup credentials, not caller overrides", async () => {
    ...
    expect(provider.requests[0]).toMatchObject({
      ...
      headers: { authorization: "Bearer startup-key", "x-startup": "startup" },
