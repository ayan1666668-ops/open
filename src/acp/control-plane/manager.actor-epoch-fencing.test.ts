...
describe("AcpSessionManager actor epoch fencing", () => {
  ...
  it.each([
    ...
    { operation: "selection commit", freshOptions: { model: "fresh-model" } },
  ])(
    ...
    async ({ operation, freshOptions }) => {
      ...
      if (operation === "option update") {
        const upsert = expectDefined(
          hoisted.upsertAcpSessionMetaMock.getMockImplementation(),
          "canonical metadata writer",
        );
        hoisted.upsertAcpSessionMetaMock.mockImplementationOnce(async (input) => {
          staleOperationEntered.resolve();
          await releaseStaleOperation.promise;
          ...
          : operation === "config option"
            ...
            : operation === "option update"
              ...
              : operation === "selection commit"
                ? manager.withExecutionSelection({
                    cfg: baseCfg,
                    sessionKey,
                    selection: store.readSelection(),
                    commitAccepted: async (_selection, assertCurrentActor) => {
                      await store.patchEntry(
                        { agentId: "codex", sessionKey },
                        async (entry) => {
                          staleOperationEntered.resolve();
                          await releaseStaleOperation.promise;
                          return { ...entry, label: "stale" };
                        },
                        { assertCommitAllowed: assertCurrentActor },
                      );
                    },
                  })
      ...
      expect(store.readOptions()).toEqual(freshOptions);
      if (operation === "selection commit") {
        expect(store.readEntry().label).toBeUndefined();
      }
  ...
  it("does not let stale status reconciliation overwrite the fresh actor metadata", async () => {
    ...
    expect(store.readMeta()?.runtimeSessionName).toBe("runtime-2");
    expect(store.readMeta()?.identity?.acpxSessionId).toBe("backend-2");
  });
  it("rejects a stale discard token without removing the successor runtime", async () => {
    ...
    installAcpSessionStoreFixture({ sessionKey, agentId: "codex" });
    const manager = new AcpSessionManager();
