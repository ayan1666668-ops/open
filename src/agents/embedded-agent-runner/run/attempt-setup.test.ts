...
describe.runIf(process.platform !== "win32")("environment-owned native attempt skills", () => {
  it.each([true, false])(
    "keeps compacted native identities with environment admission %s",
    async (enabled) => {
      const workspace = await fs.realpath(tempDirs.make("compacted-environment-skills-"));
      const names = Array.from({ length: 16 }, (_, index) => `native-${index}`);
      for (const name of names) {
        const dir = path.join(workspace, "skills", name);
        await fs.mkdir(dir, { recursive: true });
        await fs.writeFile(
          path.join(dir, "SKILL.md"),
          `---\nname: ${name}\ndescription: ${"Detailed matching guidance. ".repeat(20)}\n---\nComplete native instructions.`,
        );
      }
        ...
        overrides: {
          ...
          backend: {
            ...
            runShellCommand: vi.fn(async () => ({
              code: 0,
              stdout: Buffer.from(JSON.stringify({ platform: "linux", bins: [], env: [] })),
      ...
      const params = {
        ...
        attempt: {
          ...
          config: { skills: { entries: { remote: { enabled } } } },
        },
        effectiveWorkspace: workspace,
        sandbox,
        sessionAgentId: "main",
      };
      const native = await prepareEmbeddedSkills(params);
      native.restoreSkillEnv();
      expect(native.codeModeSkills.map((skill) => skill.name).toSorted()).toEqual(names.toSorted());
      expect(native.skillsPrompt).not.toContain("Detailed matching guidance. ".repeat(20));
      const merged = await prepareEmbeddedSkills({
        ...params,
        environmentCapabilities: [
          {
            ...
            skills: [
              {
                instructions: {
                  path: path.join(workspace, "catalog/remote/SKILL.md"),
                  contents:
      ...
      try {
        expect(merged.codeModeSkills.map((skill) => skill.name).toSorted()).toEqual(
          [...names, ...(enabled ? ["remote"] : [])].toSorted(),
        );
        expect(merged.skillsPrompt.length).toBeLessThanOrEqual(native.skillsPrompt.length + 4096);
      } finally {
        merged.restoreSkillEnv();
      }
    },
  );

  it.each(["", " \n "])("preserves an explicitly empty snapshot prompt %j", async (prompt) => {
    const workspace = await fs.realpath(tempDirs.make("empty-environment-skills-"));
    const snapshot = { prompt, skills: [] };
    const sandbox = createSandboxTestContext({
      overrides: {
        workspaceDir: workspace,
        agentWorkspaceDir: workspace,
    ...
    const prepared = await prepareEmbeddedSkills({
      includeCodeModeSkills: true,
      attempt: { skillsSnapshot: snapshot },
      effectiveWorkspace: workspace,
      sandbox,
      sessionAgentId: "main",
      environmentCapabilities: [
        {
          ...
          skills: [
            {
              instructions: {
                path: "/sandbox/catalog/audited/SKILL.md",
                contents:
                  "---\nname: audited\ndescription: Eligible remote skill\n---\nDo not admit these audited instructions.",
              },
            },
          ],
        },
    ...
    try {
      expect(prepared.skillsPrompt).toBe("");
      expect(prepared.codeModeSkills).toEqual([]);
      expect(prepared.skillsSnapshotForRun).toBe(snapshot);
    } finally {
      prepared.restoreSkillEnv();
    }
  });
