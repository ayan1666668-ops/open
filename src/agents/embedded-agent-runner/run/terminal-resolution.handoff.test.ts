...
import { resolveEmbeddedReplyPolicy } from "../../command/attempt-execution.helpers.js";
import {
...
function handoffInput(
  ...
  const producerParams = { ...handoffParams, ...runParams };
  return makeTerminalInput({
    attempt,
    runParams: {
      ...resolveEmbeddedReplyPolicy({ message: "handoff", ...producerParams }, false),
      ...producerParams,
    },
  });
}
...
describe("inter-session terminal replies", () => {
  ...
  it("keeps authored silence optional after a completed effect", async () => {
    const input = handoffInput(emptyAssistant({ content: [{ type: "text", text: "NO_REPLY" }] }));
    ...
    await expect(resolveEmbeddedRunTerminal(input)).resolves.toMatchObject({
      action: "complete",
  ...
  it("does not let authored silence waive a required handoff reply", async () => {
    const input = handoffInput(emptyAssistant({ content: [{ type: "text", text: "NO_REPLY" }] }), {
      terminalReplyExpectation: "required",
    });
    await expect(resolveEmbeddedRunTerminal(input)).resolves.toEqual({ action: "retry" });
  ...
  it.each(["pending", "delivered"] as const)(
    "does not retry a reasoning handoff whose source delivery is %s",
    async (sourceReplyDeliveryState) => {
      const input = handoffInput(
        buildEmbeddedRunnerAssistant({ content: [{ type: "thinking", thinking: "In progress." }] }),
      );
      input.attempt.sourceReplyDeliveryState = sourceReplyDeliveryState;
      await expect(resolveEmbeddedRunTerminal(input)).resolves.toMatchObject({
        action: "complete",
      });
      expect(input.activateInternalPrompt).not.toHaveBeenCalled();
