...
import type { ExecutionSelectionRequest } from "../../model-picker/execution-selection.js";
import {
  ...
  it.each(
    ...
    async ({ toolOwner, reuse }) => {
      ...
      try {
        ...
        await installSpawnModelCatalogFixture(workspaceDir);
        const executionRequest: ExecutionSelectionRequest = { kind: "initialize" };
        const preparedSelection = await supportedSpawnExecutionSelection({
          cfg: config,
          agentId: "main",
          workspaceDir,
          request: executionRequest,
          modelInput: { raw: "anthropic/claude-sonnet-4-6" },
        });
