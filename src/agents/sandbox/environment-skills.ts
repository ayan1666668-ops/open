...
export async function mergeSandboxEnvironmentSkillCatalog(params: {
  ...
  // An explicit empty snapshot is a caller-selected exclusion, not a discovery miss.
  // In particular, Skill Workshop audits must not admit the skills being reviewed.
  if (params.snapshot && !params.snapshot.prompt.trim()) {
    return { skillsPrompt: "", candidates: [] };
  }
