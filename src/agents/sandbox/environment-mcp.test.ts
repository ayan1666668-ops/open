...
describe("sandbox environment MCP", () => {
  it.each(["bundle-mcp", "remote__*"])(
    "denies authorized server %s before transport I/O",
    async (denial) => {
      const backend = createBackend();
      const start = vi.spyOn(OpenClawStdioClientTransport.prototype, "start");
      const runtime = await createRuntime(backend, {}, undefined, undefined, undefined, [denial]);
      try {
