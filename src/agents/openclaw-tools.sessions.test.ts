  ...
  describe("parent-owned ACP one-shot follow-ups", () => {
    ...
    async function prepareChild(
      ...
      readAcpSessionMetaForEntryMock.mockImplementation(
        (params: { sessionKey: string; agentId?: string; entry?: { sessionId: string } }) =>
          params.sessionKey === sessionKey &&
          params.agentId === agentId &&
          params.entry?.sessionId === entry.sessionId
            ? metadata
            : undefined,
      );
      const metadata =
    ...
    it.each([undefined, "followup"] as const)(
      ...
      async (mode) => {
        ...
        const tool = getSessionTool("sessions_send", { config, agentSessionKey: requesterKey });
    ...
    it.each(["claude", "other-owner"])(
      ...
      async (activeOwner) => {
        ...
        try {
          ...
          expect(
            callGatewayMock.mock.calls.filter(([call]) => call.method === "agent"),
          ).toHaveLength(activeOwner === "claude" ? 0 : 1);
        } finally {
          release?.();
        }
      },
