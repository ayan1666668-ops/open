...
beforeEach(() => {
  ...
  evaluateRuntimeChoiceMock
    ...
    .mockImplementation(async ({ cfg, provider, model, runtimeId }) => {
      ...
      if (!configured) return { kind: "unknown", message: "The fixture has no published model." };
...
async function runEmbeddedEntryFallback(params: {
  ...
  onPreparation?: (
    model: string,
    result: Awaited<ReturnType<typeof prepareSessionExecutionSelection>>,
  ) => void;
  ...
  if (!sessionEntry) throw new Error("Missing persisted fallback session.");
