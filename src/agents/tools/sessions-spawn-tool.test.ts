...
} from "./sessions-spawn-tool.test-support.js";

// Load after fixture mocks register, independent of static import sorting.
const { createAgentsWaitTool } = await import("./agents-wait-tool.js");
  ...
  it("creates visible sessions while carrying inherited tool restrictions forward", async () => {
    ...
    expect(hoisted.inProcessCreationMock).toHaveBeenCalledWith(
      ...
      }),
      {
        ...
        spawnModelAutoSelection: { model: "mock-provider/primary", hasFallbackOrigin: true },
        inheritedToolPolicy: {
          version: 1,
