...
Authenticated Control UI administrator turns can list/get/update/run/remove/runs any Gateway automation. Other turns have a restricted inventory; use a fresh admin Control UI turn or the Automations page for cross-session management.
...
export function createCronTool(opts?: CronToolOptions, deps?: CronToolDeps): AnyAgentTool {
  ...
  const tool: AnyAgentTool = {
    ...
    description: managementAuthority?.managementOnly
      ? 'Manage any existing automation on this Gateway as the authenticated Control UI administrator. Actions: list [includeDisabled,limit,offset] (compact summaries with timing; follow nextOffset); get jobId (full schedule, payload, and delivery details); update jobId job (partial patch, null clears); run jobId (runMode:"force" runs now); remove jobId; runs jobId (run history). Creator attribution and scheduled execution policy stay intact. Use the Automations page for other actions.'
    ...
    execute: async (_toolCallId, args, operationSignal) => {
      ...
      if (
        ...
        throw new Error(
          "This turn can only list, get, update, run, remove, or inspect run history for automations. Use the Automations page for other actions.",
        );
      }
