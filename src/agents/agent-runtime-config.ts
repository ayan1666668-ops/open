...
export async function resolveAgentRuntimeConfig(
  ...
  const prepareSecretsSnapshot = async (snapshotParams?: {
    pinnedProfileId?: string;
    configBoundProfileIds?: ReadonlySet<string>;
  }): Promise<void> => {
    ...
    const snapshot = await measureAgentStartup(
      ...
      () =>
        secretsRuntime.prepareSecretsRuntimeSnapshot({
          ...
          ...(snapshotParams?.pinnedProfileId
            ? { pinnedProfileId: snapshotParams.pinnedProfileId }
            : {}),
          ...(snapshotParams?.configBoundProfileIds
            ? { configBoundProfileIds: snapshotParams.configBoundProfileIds }
            : {}),
        }),
      { config: cfg },
    );
