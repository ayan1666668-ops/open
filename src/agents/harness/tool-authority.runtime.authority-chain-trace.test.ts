...
describe("retired-operation fallback: authority-chain terminal trace (#139847)", () => {
  it("produces the redacted evidence trace under controlled fault injection", async () => {
    ...
    log(
      "1 identify",
      "captured reply operation",
      "retired via complete() before bind (controlled fault injection)",
    );
      ...
      withPreparedEmbeddedRunToolAuthority(
        ...
        async (prepared) => {
          ...
          try {
            ...
            // Close the admission first, then republish an otherwise-valid run
            // handle (matching run id and the prepared fingerprint), so the only
            // possible rejection is the closed lifecycle itself — a fingerprint
            // or registration mismatch cannot mask an ineffective guard here.
            ...
            // The revocation coverage claim must fail this trace when the guard
            // is ineffective: only a real lifecycle rejection counts.
            expect(revokeResult.startsWith("rejected")).toBe(true);
            expect(revokeResult).toContain("no longer active");
            expect(revokeResult).not.toContain("does not match its attempt");
          } finally {
