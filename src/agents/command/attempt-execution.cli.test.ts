...
vi.mock("../model-runtime-choice.js", () => ({
  evaluatePublishedModelRuntimeChoice: async ({
    ...
    const expectedRuntime =
      provider === "anthropic" &&
      ["fixture-primary", "fixture-fallback", "claude-sonnet-4-6"].includes(model)
        ? "claude-cli"
        : provider === "custom" && ["base", "child"].includes(model)
          ? "openclaw"
          : undefined;
    if (!expectedRuntime) {
      return { kind: "unknown", message: "Undeclared outer-fallback fixture selection." };
    }
    // These fixture routes have one compatible executor, so fallback exercises
    // the compiler's unsupported-runtime transition instead of switching a sticky app.
    if (runtimeId !== expectedRuntime) {
      return { kind: "unsupported", message: "Fixture route requires its declared executor." };
    }
...
describe("CLI attempt execution", () => {
  ...
  it.each([
    ...
    "live model switch",
    "initialized CLI selection",
  ] as const)("retains CLI image capability after a thinking-off %s", async (transition) => {
    const initializedCli = transition === "initialized CLI selection";
    const model = "claude-sonnet-4-6";
    const modelRef = `anthropic/${model}`;
    const cliSelection: ModelExecutionSelection = {
        ...
        : initializedCli
          ...
          : {
              executionSelection: {
                state: "accepted",
                selection: {
                  model: { provider: "custom", id: "child" },
    ...
    const cfg: OpenClawConfig = {
      ...
      agents: {
        ...
        defaults: {
          model: {
            primary: implicitPrimary || initializedCli ? modelRef : "custom/base",
            ...(transition === "configured fallback" ? { fallbacks: [modelRef] } : {}),
          },
          modelPolicy: {
            allow: ["custom/base", "custom/child", modelRef],
          },
    ...
    await fs.writeFile(imagePath, createSolidPngBuffer(1, 1, { r: 0, g: 0, b: 0 }));
    await writeSessionStoreSeed(sessionStore);
    if (!initializedCli) {
      runEmbeddedAgentMock.mockImplementationOnce(async () => {
        if (transition === "live model switch") {
          const next = { ...expectDefined(sessionStore[sessionKey], "live-switch session") };
          commitSessionExecutionSelection(next, cliSelection);
          await replaceSessionEntry({ sessionKey, storePath }, next);
          sessionStore[sessionKey] = next;
          throw new LiveSessionModelSwitchError({ selection: cliSelection });
        }
        throw new FailoverError("Configured child capacity", {
          reason: "rate_limit",
          provider: "custom",
          model: "child",
        });
    ...
    if (initializedCli) {
      ...
      expect(readSessionStore()[sessionKey]?.executionSelection).toMatchObject({
        state: "accepted",
