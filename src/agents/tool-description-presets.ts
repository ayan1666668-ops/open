...
export function describeSessionsListTool(options?: SessionLinkDescriptionOptions): string {
  return [
    ...
    "Metadata-only by default. limit defaults to 100 (max 200); count is this page, never an inventory total. Continue with nextOffset and identical filters while hasMore; truncationReason names a scan/byte budget. Pages are live, not a frozen snapshot: concurrent changes can move rows; deduplicate by agentId/key/sessionId or restart for a fresh inventory. archived=all includes active and archived rows.",
