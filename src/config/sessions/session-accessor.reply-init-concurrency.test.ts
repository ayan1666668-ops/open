...
import { createConcurrencyWorkerScript } from "./session-accessor.reply-init-concurrency.test-support.js";
...
async function getConcurrencyWorker(): Promise<ReturnType<typeof spawn>> {
  ...
  const child = spawn(
    ...
    [
      ...
      "--input-type=module",
      "--eval",
      createConcurrencyWorkerScript(sessionAccessorUrl, SESSION_KEY, AGENT_ID),
    ],
