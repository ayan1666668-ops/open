...
function hasRetainedSessionPendingInputNative(
...
function readSessionSubmittedInputNative(
  scope: PendingInputScope,
...
export function readSessionPendingSourceNative(
  scope: PendingInputScope,
  request: import("./session-transcript-worker.types.js").SessionPendingSourceWorkerInput["request"],
) {
  if (request.kind === "submitted") {
    return readSessionSubmittedInputNative(scope, request.idempotencyKey);
  }
  if (request.kind === "retained") {
    return hasRetainedSessionPendingInputNative(scope, request);
  }
  const resolved = resolveSqliteTranscriptScope(scope);
  const result = withOpenClawAgentDatabaseReadOnly(
    (database) =>
      runSqliteDeferredTransactionSync(database.db, () => {
        ...
        const row = executeSqliteQueryTakeFirstSync(
          ...
          getSessionKysely(database.db)
            ...
            .where("message_json", "=", request.messageJson)
