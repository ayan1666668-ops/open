...
describe("session accessor seam", () => {
  ...
  it("preserves configured-default intent across reopen and unrelated whole-entry writes", async () => {
    ...
    closeOpenClawAgentDatabasesForTest();
    const reopenedEntry = expectDefined(
      loadSessionEntry({ sessionKey: childKey, storePath }),
      "reopened child entry",
    );
    await replaceSessionEntry(
      { sessionKey: childKey, storePath },
      { ...reopenedEntry, label: "preserved by unrelated writer" },
    );

    closeOpenClawAgentDatabasesForTest();
    const reopenedChild = expectDefined(
      loadSessionEntry({ sessionKey: childKey, storePath }),
      "rewritten child entry",
    );
    expect(reopenedChild).toMatchObject({
      label: "preserved by unrelated writer",
      executionSelection: {
        state: "deferred",
        request: { defaultSelection: "configured" },
        fallbackPermission: "configured",
    ...
    expect(
      resolveStoredModelOverride({
        ...
        sessionEntry: projectPublicSessionEntry(reopenedChild),
        sessionKey: childKey,
  ...
  it.each([
    ...
    },
    {
      ...
      initial: {},
      concurrent: {
        executionSelection: acceptedModelSelection("openai", "channel-model", {
          executor: { kind: "harness", id: "codex" },
        }),
        authProfileOverride: "openai:channel",
  ...
  it("replaces entries so deleted fields stay removed", async () => {
    ...
    expect(loadSessionEntry(scope)?.executionSelection).toBeUndefined();
  });
    ...
    await patchSessionEntryCore(
      ...
      (entry, context) => {
        ...
        return {
          ...
          executionSelection: {
            state: "deferred",
            request: { model: { provider: "openai", id: "gpt-5.5" } },
            fallbackPermission: "explicit",
          },
        };
      },
