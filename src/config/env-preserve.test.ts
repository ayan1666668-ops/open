...
describe("stack-safe deep env-ref restoration", () => {
  ...
  it("restores deep paths from the env-ref map without a call-stack overflow", () => {
    const depth = 5_000;
    const value = buildNestedObject(depth, { leaf: "resolved-value" });
    const leafPath = `${Array(depth).fill("level").join(".")}.leaf`;
    const result = restoreEnvRefsFromMap(
      value,
      "",
      new Map([[leafPath, "${OPENCLAW_PROBE_VAR}"]]),
      new Set<string>(),
    );
    expect(readDeepLeaf(result, depth)).toEqual({ leaf: "${OPENCLAW_PROBE_VAR}" });
  });

  it("keeps a deep map-restored document identical when nothing matches", () => {
    const depth = 5_000;
