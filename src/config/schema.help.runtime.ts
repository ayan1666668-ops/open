...
export const RUNTIME_FIELD_HELP: Record<string, string> = {
  ...
  "skills.workshop.model":
    "Provider/model ref or alias used for background Skill Workshop reviews (the experience review after deep turns). Defaults to the reviewed turn's own model, which keeps prompt-cache affinity on that provider. Point it at a cheaper or hosted model to keep review traffic off a local inference server; an unresolvable ref logs a warning and falls back to the default.",
  approvals:
