...
describe("auth-profile store ref resolution (issue #145740)", () => {
  ...
  it("keeps source isolation when config and auth-store collide on the same store key (P1-B)", async () => {
    // Both the shared owner store and the temporary run store hold a value for
    // the SAME store:provider:id. A config reference (models.providers.<id>.apiKey)
    // and an auth-profile keyRef share that key. The auth-store assignment must
    // resolve through the owner store; the config assignment must keep the
    // temporary run store — the config consumer must never receive the shared
    // owner credential.
    const ownerStateDir = tempDirs.make("t16-owner-state-");
    const tempStateDir = tempDirs.make("t16-temp-state-");
    const mainAgentDir = tempDirs.make("t16-agent-");
    const SHARED_KEY = "SHARED_API_KEY";
    ...
    writePersistedAuthProfileStoreRaw(
      {
        ...
        profiles: {
          "shared:test": {
            type: "api_key",
            provider: "shared",
            keyRef: { source: "store", provider: "default", id: SHARED_KEY },
          },
        },
      },
      mainAgentDir,
    ...
    const config = asConfig({
      auth: { order: { shared: ["shared:test"] } },
      models: {
        providers: {
          shared: {
            apiKey: { source: "store", provider: "default", id: SHARED_KEY },
          },
        },
      },
    });
    ...
    // The auth-store profile resolves through the owner store, NOT the temp value.
    const authProfile = snapshot.authStores
      .map((entry) => entry.store.profiles["shared:test"])
      .find((profile): profile is ApiKeyCredential => profile?.type === "api_key");
    expect(authProfile?.key).toBe("owner-shared");

    // The temporary run store still holds its own value; it was not overwritten
    // by the shared credential (the config consumer kept the run-store value).
    const tempRead = readSecretStoreValue({
      scope: { kind: "team" },
      name: SHARED_KEY,
      database: { env: { OPENCLAW_STATE_DIR: tempStateDir } },
    });
    expect(tempRead.ok).toBe(true);
    if (tempRead.ok) {
      expect(tempRead.value).toBe("temp-shared");
    }
  });
});
