...
decision-model API, plus the optional provider-neutral `decision_evaluate` tool.
...
`typesafe/jev-latest` as your agent's `decisionModel`. `decision_evaluate` sends the
supplied evidence to TypeSafe AI and incurs its normal usage charges.
