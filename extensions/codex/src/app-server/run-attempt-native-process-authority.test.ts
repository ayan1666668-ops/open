...
async function fixture(
  options: {
    ...
    nativeHookRelay?: { enabled: boolean; events?: readonly NativeHookRelayEvent[] };
  } = {},
) {
    ...
    const run = runCodexAppServerAttempt(params, {
      pluginConfig: {
        appServer: {
          ...
          ...(options.nativeHookRelay ? { loopDetectionPreToolUseRelay: false } : {}),
        },
      },
      ...(options.nativeHookRelay ? { nativeHookRelay: options.nativeHookRelay } : {}),
    });
...
describe("native background process source authority", () => {
  it.each([
    { name: "disabled optional relays", enabled: false, events: [] },
    { name: "explicit post-tool scope", enabled: true, events: ["post_tool_use"] },
  ] as const)("keeps native process admission with $name", async ({ enabled, events }) => {
    const createRelay = vi.spyOn(nativeHookRelay, "createCodexNativeHookRelay");
    const f = await fixture({ nativeHookRelay: { enabled, ...(enabled ? { events } : {}) } });
    try {
      const staff = await f.begin("maintainer");
      const start = f.harness.requests.find(({ method }) => method === "thread/start")
        ?.params as CodexThreadStartParams;
      const relay = nativeHookRelayTesting.getNativeHookRelayRegistrationForTests(
        extractRelayIdFromThreadRequest(start),
      );
      const handle = createRelay.mock.results.at(-1)?.value;
      expect(relay).toBeDefined();
      expect(handle?.relayId).toBe(extractRelayIdFromThreadRequest(start));
      expect(start.approvalPolicy).toBe("never");
      expect(start.config?.["hooks.PreToolUse"]).toEqual([
        expect.objectContaining({ matcher: "Bash|exec|exec_command", hooks: [expect.any(Object)] }),
      ]);
      expect(relay?.allowedEvents).toEqual([...events, "pre_tool_use"]);
      expect(start.config).not.toHaveProperty(["hooks.Stop"]);
      expect(start.config).not.toHaveProperty(["hooks.PermissionRequest"]);
      if (!enabled) {
        expect(start.config).not.toHaveProperty(["hooks.PostToolUse"]);
      }
      // The installed command and matcher must describe the live admission owner.
      expect(start.config).toMatchObject(
        buildCodexNativeHookRelayConfig({ relay: handle!, events: relay!.allowedEvents }),
      );
