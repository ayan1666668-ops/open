  ...
  it.each(["thread/closed", "client-close", "forget"] as const)(
    ...
    async (terminal) => {
      ...
      try {
        ...
        expect(invalidated).not.toHaveBeenCalled();
        expect(readCodexNativeHookInstallation(harness.client, "thread-terminal")).toBe("installed");
        for (let attempt = 0; attempt < 2; attempt++) {
          ...
          expect(invalidated).toHaveBeenCalledOnce();
          expect(readCodexNativeHookInstallation(harness.client, "thread-terminal")).toBeUndefined();
          expect(readCodexEphemeralThreadCatalog(harness.client, "thread-terminal")).toBeUndefined();
        }
        expect(release).toHaveBeenCalledOnce();
