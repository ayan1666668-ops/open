...
import { registerClientRuntimeOwnershipTests } from "./client-runtime-ownership.test-support.js";
...
const runtime = await import("./client-runtime.js");
const creations = await import("./client-runtime-state.js");
const {
...
describe("Codex app-server client runtime", () => {
  ...
  registerClientRuntimeOwnershipTests(
    runtime,
    creations,
    clients,
    mocks,
    EXPECTED_LIVE_THREAD_IDLE_TIMEOUT_MS,
    EXPECTED_MAX_IDLE_LIVE_THREADS,
  );
  ...
  it("does not let an older owner forget or release a newly claimed thread generation", async () => {
    ...
    ensureCodexAppServerClientRuntime(client, { agentDir: "/tmp/agent" });
    recordCodexEphemeralThreadCreation(client, "thread-reclaimed", {
    ...
    expect(readCodexNativeHookInstallation(client, "thread-reclaimed")).toBe("installed");
    expect(readCodexEphemeralThreadCatalog(client, "thread-reclaimed")).toEqual([]);
  ...
  it.each(["thread/closed", "client-close", "forget"] as const)(
    ...
    async (terminal) => {
      ...
      clients.push(harness.client);
      ensureCodexAppServerClientRuntime(harness.client, { agentDir: "/tmp/agent" });
      recordCodexEphemeralThreadCreation(harness.client, "thread-terminal", {
        hookInstallation: "installed",
        dynamicTools: [],
      });
      ...
      try {
        ...
        expect(invalidated).not.toHaveBeenCalled();
        expect(readCodexNativeHookInstallation(harness.client, "thread-terminal")).toBe(
          "installed",
        );
        for (let attempt = 0; attempt < 2; attempt++) {
          ...
          expect(invalidated).toHaveBeenCalledOnce();
          expect(
            readCodexNativeHookInstallation(harness.client, "thread-terminal"),
          ).toBeUndefined();
          expect(
            readCodexEphemeralThreadCatalog(harness.client, "thread-terminal"),
          ).toBeUndefined();
        }
        expect(release).toHaveBeenCalledOnce();
