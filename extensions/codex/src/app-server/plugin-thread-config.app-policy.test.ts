...
describe("Codex plugin app policy", () => {
  it.each([
    ["prompt", "user"],
    ["prompt", "auto_review"],
    ["approve", "user"],
  ...
  it.each(["prompt", "approve"])(
    "enforces destructive denial before native %s approval on initial and replayed threads",
    async (approvalMode) => {
        ...
        apps: {
          "google-calendar-app": {
            ...
            tools: {
              ...
              policy_delete: { enabled: true, approval_mode: "approve" },
              retired: { enabled: true },
              native_disabled: { enabled: false },
              safe_write: { approval_mode: "approve" },
            },
          },
        },
      };
