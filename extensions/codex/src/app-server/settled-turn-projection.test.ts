...
function projectedCallIds(messages: AgentMessage[]): string[] {
  ...
  const prior = new SettledTurnPriorContext(seen);
  const current = projectSettledCodexMessages(messages, seen);
  return prior
    .prependTo(current)
    .filter(
...
describe("projectSettledCodexMessages", () => {
  ...
  it("rewrites overlength call ids in prior turn groups and keeps their pair intact", () => {
    const seen = new Set<string>();
    const prior = new SettledTurnPriorContext(seen);
    for (const entry of [
      message({ role: "user", content: "Send the earlier update." }),
      toolCall(OVERLENGTH_CALL_ID),
      toolResult(OVERLENGTH_CALL_ID),
    ]) {
      prior.append(entry);
    ...
    const current = projectSettledCodexMessages(
      [message({ role: "user", content: "Send the next update." }), toolCall(), toolResult()],
      seen,
    );
    const items = prior.prependTo(current);
    ...
    expect(ids[0]).toMatch(/^call_[A-Za-z0-9_-]{1,59}$/);
    expect(ids[0]).toBe(ids[1]);
    expect(ids.slice(2)).toEqual(["call-1", "call-1"]);
  });

  it("reserves a current raw id that equals a prior overlength id's rewrite", () => {
    const [collidingId] = projectedCallIds([
      toolCall(OVERLENGTH_CALL_ID),
      toolResult(OVERLENGTH_CALL_ID),
    ]);
    ...
    const current = projectSettledCodexMessages(
      [
        message({ role: "user", content: "Send the next update." }),
        toolCall(collidingId as string),
        toolResult(collidingId as string),
      ],
      seen,
    ...
    expect(ids).toHaveLength(4);
    expect(ids.slice(2)).toEqual([collidingId, collidingId]);
    expect(ids[0]).toBe(ids[1]);
    expect(ids[0]).not.toBe(collidingId);
    expect(ids[0]).toMatch(/^call_[A-Za-z0-9_-]{1,59}$/);
    expect(ids.every((id) => id.length <= 64)).toBe(true);
