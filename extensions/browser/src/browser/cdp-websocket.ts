...
export async function withCdpSocket<T>(
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    // Cancellation must own the socket for the whole operation, not just the
    // handshake: an unanswered in-flight CDP command has no other cancel path
    // and can hold gateway drain open past every cleanup budget.
