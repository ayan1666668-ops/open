...
describe("install.ps1 failure handling", () => {
  ...
  beforeAll(() => {
    ...
    const cases = [
      ...
      {
        name: "winget-stale-registration-repair",
        source: [
          scriptWithoutEntryPoint,
          "",
          "$script:ready = $false",
          ...
          "  if ($args[0] -eq 'install') { $global:LASTEXITCODE = -1978335189; return }",
          "  if ($args[0] -eq 'repair') {",
          "    $global:LASTEXITCODE = $script:repairExit",
          "    if ($script:repairExit -eq 0) { $script:ready = $true }",
          "    return",
          "  }",
          '  throw "Unexpected winget call: $args"',
          "}",
          "function Refresh-ProcessPath { }",
          "function Add-InstalledNodeToProcessPath { return $true }",
          "function Check-Node { return $script:ready }",
          "function Install-PortableNode { $script:portableCalls += 1; $script:ready = $true }",
          "$result = @(Install-Node)",
          'if ($result.Count -ne 1 -or $result[0] -ne $true) { throw "Install-Node returned $result" }',
          "$expected = @(",
          "  'install OpenJS.NodeJS.LTS --source winget --accept-package-agreements --accept-source-agreements',",
          "  'repair --id OpenJS.NodeJS.LTS --exact --source winget --accept-package-agreements --accept-source-agreements'",
          ") -join '|'",
          "if (($script:wingetCalls -join '|') -ne $expected) { throw \"Calls=$($script:wingetCalls -join '|')\" }",
          "if ($script:portableCalls -ne 0) { throw 'Portable fallback ran after successful repair' }",
          "$script:ready = $false",
          "$script:repairExit = 1",
          "$script:portableCalls = 0",
          "$script:wingetCalls = @()",
          "$result = @(Install-Node)",
          'if ($result.Count -ne 1 -or $result[0] -ne $true) { throw "Failed repair result=$result" }',
          "if (($script:wingetCalls -join '|') -ne $expected) { throw \"Failed repair calls=$($script:wingetCalls -join '|')\" }",
          'if ($script:portableCalls -ne 1) { throw "Portable calls=$script:portableCalls" }',
          "",
        ].join("\n"),
      },
      {
...
`,
  ...
  runIfPowerShell("upgrades and validates Node installed by Windows package managers", () => {
    ...
    expectBatchedPowerShellCase("winget-stale-registration-repair");
  });
