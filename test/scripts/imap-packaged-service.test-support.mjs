...
try {
  ...
  if (expected === "red") {
    ...
    await completed.promise;
    const restartLogs = logs.slice(restartLogOffset);
    assert(restartLogs.some((text) => text.includes("lastSweep=")));
    assert(!restartLogs.some((text) => text.includes("sweep failed=")));
