...
node scripts/e2e/lib/update-first-hop-package-fixtures.mjs \
  future-tarball "$candidate_package" "$future_package" \
  >/tmp/openclaw-corrupt-plugin-update-method.json
...
# Keep unpublished candidate dependencies reachable without retaining the
# one-shot plugin fixture registry that made the corrupt payload available.
export NPM_CONFIG_REGISTRY="${OPENCLAW_PREPUBLISH_PLUGIN_REGISTRY_URL:-https://registry.npmjs.org/}"
...
node scripts/e2e/lib/release-scenarios/assertions.mjs \
  assert-package-version "$package_root" "$future_version" same-schema-update
