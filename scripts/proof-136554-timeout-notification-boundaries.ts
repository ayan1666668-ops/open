 ...
 *
 * Adapted from our #126924 notification proof. Adds real cancellation waits that
 * assert the disposition the wait manager itself records. This exercises
 * notification settlement, not process liveness, restart/upgrade, or deletion
 * retention.
 *
 * The `silent_dm` settlement scenario is the review-flagged shape: a
 * completion-enabled direct-message parent that answers a provisional expiry
 * with NO_REPLY must settle the notification rather than re-announce forever.
 *
 * Deliberately NOT proven here: the parent-facing announcement that kill
 * reconciliation releases after its 5-minute provisional-kill grace. That path
 * reads the child transcript through a real Gateway `chat.history` call whose
 * dependency seam is published only under Vitest, so it cannot be scripted from
 * a plain process without changing the runtime under test. The killed
 * disposition reaching the parent announcement is covered by the Vitest case
...
try {
  ...
  // Each cancellation carries the wait status its stop reason actually presents:
  // `rpc` is a cancellation only on a non-ok wait (a model/ACP "stop" that ends a
  // successful wait is an ordinary completion), while `aborted` and `superseded`
  // are cancellations from their reason alone.
  const cancellations = new Map<string, { stopReason: string; status: "ok" | "error" }>();
    ...
    for (const response of ["silent", "error"] as const) {
      ...
      if (disposition === "still-running") {
        ...
        if (response === "error") {
          ...
          // Instructed silence settles. A retryable result here would make the
          // wait manager re-announce every few seconds while the child works.
  ...
  for (const { stopReason, status } of [
    ...
    const runId = `cancelled-${stopReason}`;
    const childSessionKey = `agent:main:subagent:${runId}`;
    const requesterSessionKey = `${slackRequester}-${runId}`;
    const startedAt = Date.now() - 200;
    const taskCreatedAfter = Date.now();
    waitStarts.set(runId, startedAt);
    cancellations.set(runId, { stopReason, status });
    await sessions.replaceSessionEntry(
      { sessionKey: requesterSessionKey, agentId: "main" },
      { sessionId: `requester-${runId}`, updatedAt: Date.now() },
    );
    await sessions.replaceSessionEntry(
      { sessionKey: childSessionKey, agentId: "main" },
      { sessionId: `session-${runId}`, updatedAt: Date.now(), status: "running", startedAt },
    );
    registry.registerSubagentRun({
      ...
      task: "cancelled child reports killed, not exited",
      cleanup: "keep",
      expectsCompletionMessage: true,
      // The cancellation must reconcile a real durable task row, not a stub.
      taskRowOwnership: "required",
    });
    const run = () =>
      read.listSubagentRunsForRequester(requesterSessionKey).find((row) => row.runId === runId);
    const cancelledTaskStatus = () => {
      const found = tasks.findDetachedTaskRun({
        runId,
        runtime: "subagent",
    ...
    assert.equal(cancelledTaskStatus(), "running", "launch owns a real SQLite task");
    await until(
      `${stopReason}: cancellation observed`,
      () => run()?.endedReason === "subagent-killed",
    );
    // This is the disposition the waiter itself produced. Before the repair the
    // wait manager recorded the killed *reason* while building an outcome with no
    // disposition at all, so the total read defaulted it to `exited` and the
    // parent could not tell a killed child from one that exited on its own.
    assert.equal(run()?.execution.outcome?.disposition, "killed");
    assert.equal(run()?.execution.outcome?.status, "error");
    assert.equal(run()?.execution.outcome?.error, "subagent run terminated");
    assert.equal(run()?.suppressAnnounceReason, "killed");
    assert.ok(run()?.killReconciliation, "a provisional kill reconciliation is recorded");
    assert.equal(cancelledTaskStatus(), "cancelled", "the durable task row records cancellation");
  ...
  for (const mode of [
    ...
    requesterResponses.set(requesterSessionKey, (params) => {
      if (!silent) {
        assert.equal(
          params.deliver,
          true,
          "registry proof must reach the automatic external route",
        );
      }
      if (completing) {
    ...
    process.stdout.write(
      `[pass] registry ${mode}: notification settled, same child later reported its real result\n`,
    );
  }
  process.stdout.write(
