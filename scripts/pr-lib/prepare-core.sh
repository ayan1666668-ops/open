...
resolve_pr_author_access_at_prepare() {
  ...
  local author="$1" repo_nwo="$2" response permission exit_code
  if response=$(pr_gh_plain api "repos/$repo_nwo/collaborators/$author/permission") &&
...
prepare_init() {
  ...
  author_access_at_prep=$(resolve_pr_author_access_at_prepare "${PR_AUTHOR:-}" "$(printf '%s\n' "$json" | jq -r .baseRepository.nameWithOwner)") || return 1
