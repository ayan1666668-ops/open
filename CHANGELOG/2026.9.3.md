<!-- openclaw-docs-mirror-v1 {"version":"2026.9.3","sources":["docs/releases/2026.9.3.md"],"sourceDigest":"sha256:c1f7d9d9d1e3e43d68d8c147a697d24e133a16e25980a7ab6d540cc5c1ffc5b3"} -->
...
> **Note**
> 
> Upgrade Node before OpenClaw if you use an older runtime. This release requires Node 24.16.0 or newer on 24.x, or Node 26.1.0 or newer. Node 26 is recommended. See the [Node requirements](https://docs.openclaw.ai/install/node) for supported hosts.
...
![Conceptual 2026.9.3 explainer: Included in the public link — Existing and future conversation text — Anyone with the link can read; Left out — Tools, reasoning, files and images — No executable widgets. Creator or admin can revoke access. Revocation cannot recall copies that readers saved.](https://github.com/user-attachments/assets/3d3a80fc-9a47-4272-a22d-b771382deabc)

_A public read-only transcript is an explicit publishing decision. Conceptual illustration, not a screenshot. Creator or admin can revoke access. Revocation cannot recall copies that readers saved._
...
##### Keep the sidebar readable during activity
...
Eligible updates driven by 2026.9.2 can cross a shared-state schema change while the older updater finishes reading its update history. Agent-schema migrations, missing state metadata and failed state migrations still require the documented external-install and fresh-Doctor recovery path. See [schema-bump recovery](https://docs.openclaw.ai/reference/database-schemas#schema-bumps-and-older-updaters) if your update is refused.
...
##### Switch installation channels and align official plugins
...
##### Recover service connections on Mac and Windows
...
A skill your agent learns now belongs to that agent’s [Workshop](https://docs.openclaw.ai/tools/skill-workshop), so moving between workspaces no longer means moving the collection along with it. Installed skills and suggestions have separate views, keeping what the agent can already use distinct from changes awaiting review.
...
![Conceptual 2026.9.3 explainer: Cedar’s Workshop — Volunteer-brief checklist — One agent-owned collection; Harbor workspace — Prepare the opening — Uses Cedar’s learned skill; Orchard workspace — Plan a later event — Uses the same collection. Installed skills and suggestions stay separate. Ambiguous legacy copies remain for review.](https://github.com/user-attachments/assets/f0817ba6-886d-4f20-9e56-8c11df2cb18d)

_Changing workspaces no longer means moving learned instructions. Conceptual illustration, not a screenshot. Installed skills and suggestions stay separate. Ambiguous legacy copies remain for review._
...
##### Read and review complete skill instructions
...
##### Follow weekly collection reviews
...
![Conceptual 2026.9.3 explainer: Supported account order — Inspect source and health — Change or clear saved priority; One-account sign-out — Other accounts stay connected — Missing explicit choices need recovery. Inherited and provider-managed orders retain restrictions. This is not a guarantee of provider availability.](https://github.com/user-attachments/assets/d7977359-c0ad-48e8-90bd-46db5dcf5211)

_Administrative account controls for the selected agent. Conceptual illustration, not a screenshot. Inherited and provider-managed orders retain restrictions. This is not a guarantee of provider availability._
...
[LM Studio](https://docs.openclaw.ai/providers/lmstudio) now budgets against the context of the loaded model instance, and with preload enabled sends chat to an instance that has enough room. If a new instance is loaded, the request uses its returned identity while your configured model reference stays the same. This avoids budgeting for a larger instance and then sending the conversation to a smaller one.
...
##### Continue CLI-backed conversations
...
The [TaskFlow examples](https://docs.openclaw.ai/automation/taskflow) now use supported [Lobster](https://docs.openclaw.ai/tools/lobster) approval and resume flows, with child work linked through its authoritative owner. These are bounded examples using prepared synthetic batches; connecting a real inbox or pull-request source still requires its own adapter.
...
The [Browser panel](https://docs.openclaw.ai/tools/browser) shows page repaints as they happen, so you can follow a page loading, scrolling or responding without waiting for the next screenshot. Existing click, type, scroll, Annotate and Inspect controls remain available.
...
##### Update plugins for the current SDK
...
##### Load installed plugins and preserve recovery copies
...
[Code Mode](https://docs.openclaw.ai/tools/code-mode) can keep a program running through fast tool replies and optionally check composed TypeScript calls before execution. Long conversations, session lists, delegated work, and terminal diagnostics also receive improvements that make their results easier to retain and inspect.
