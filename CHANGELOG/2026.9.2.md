<!-- openclaw-docs-mirror-v1 {"version":"2026.9.2","sources":["docs/releases/2026.9.2.md"],"sourceDigest":"sha256:00bfc6d55f6a97acbef9f32c6914115c0136efae888c1132796cfe8d1083a6da"} -->
...
Before [local configuration changes](https://docs.openclaw.ai/cli/configure), OpenClaw also checks the installed Gateway's recorded data and configuration paths, even when the service is stopped. A confirmed mismatch stops the wizard before it writes to the wrong location. If those paths cannot be verified, configuration remains available with a warning and a pointer to `openclaw gateway status --deep`.
...
##### Start with an OpenAI key


Clean, environment-only [agent runs](https://docs.openclaw.ai/cli/agent#agent-exec) using an OpenAI API key can now start through the built-in OpenClaw runtime when the optional Codex runtime is absent. This removes a setup failure before the agent could answer or begin its task, while explicitly chosen runtimes and existing session pins continue to require their selected runtime.
...
##### Recover shell completion setup


When Doctor or onboarding cannot update your shell profile, [completion setup](https://docs.openclaw.ai/cli/completion) now provides a command that loads the prepared completions in your current shell instead of sending you back into the same permission failure. Run the complete command it displays; enabling completion for future shells still requires fixing the reported permission or read-only error and retrying installation. PowerShell recovery commands also handle paths containing typographic apostrophes.
...
##### Find dashboards in a visual gallery
...
##### Build and open a session dashboard
...
##### Use interactive widgets in chat
...
##### Recognize people and assign conversations
...
##### Review files and follow GitHub work
...
##### Control the community invitation
...
![Published 2026.9.2 New Session interface with a fictional Harbor volunteer brief, Local placement and Agent workspace.](https://github.com/user-attachments/assets/893aa22a-fab5-4712-9c32-a3dfff6ec804)

_A clear brief and an explicit workspace before the first message. Published 2026.9.2 interface with fictional Harbor data; no live task execution._
...
##### Expand a table without losing its links
...
##### Complete package and source updates
...
##### Keep working settings during upgrades
...
![Conceptual 2026.9.2 explainer: Next relevant operation — Routine tools, speech and retention — Affected services may reconnect; Full restart still needed — Listener and process settings — Authentication mode and asset roots. Refresh open pages for UI or terminal-policy changes. Failed service replacement can need recovery.](https://github.com/user-attachments/assets/4ad93205-6ad2-43e6-8931-a5c1a9079c31)

_Hybrid reload keeps unaffected work connected. Conceptual illustration, not a screenshot. Refresh open pages for UI or terminal-policy changes. Failed service replacement can need recovery._
...
##### Continue eligible work after restart
...
##### Stop the Gateway with accurate cleanup status
...
##### Archive and clean up large conversation stores
...
##### Get repair advice for the right installation
...
##### Start managed services and migrated installs
...
##### Attempt a bounded installation repair
...
##### Finish Slack replies without replaying uncertain sends
...
##### Use tables and decision buttons in Slack
...
##### Keep Discord voice with the speaker
...
##### Answer a waiting agent question
...
##### Use the selected memory plugin
...
##### Read and validate Memory Wiki pages


Once your [Memory Wiki](https://docs.openclaw.ai/cli/wiki) vault is active, opening an existing visible page by its exact Markdown path reads that page without scanning unrelated pages, so a broken file elsewhere no longer has to block the page you asked for. Local source sync can also reuse an existing compiled Wiki after startup. Fresh CLI processes still validate the saved snapshot, and lookups by a short name, page ID, or missing path can still need a vault scan.
...
##### Inspect and preserve saved memory


When a memory database grows, explicit memory status checks now show its file size, transaction log, reusable space, and retained embedding cache, including cache entries left after caching was disabled. Those measurements overlap, so they should not be added together as separate pieces of disk usage. The [disk-space recovery guide](https://docs.openclaw.ai/concepts/memory-builtin#reclaim-disk-space) explains how to use the existing offline compaction command with a verified backup and stopped writers, preserving sessions and allowing you to skip an index reset when you only need to reclaim unused pages.
...
##### Keep removed tool context available to compaction
...
##### Validate and recover Workshop changes


[Skill Workshop](https://docs.openclaw.ai/tools/skill-workshop) now identifies conflicting support-file locations before saving a proposal, and refuses proposals or evaluations that would silently omit unreadable or excessively nested content. When agents sharing a workspace cannot start a collection review because a provider credential is missing, the error names the affected provider and agent so the operator can fix authentication without guessing at model settings.

Restoring an older collection backup also protects later edits and deliberate deletions when the relevant files cannot be verified. Some older backups need [manual recovery](https://docs.openclaw.ai/tools/skill-workshop#when-an-older-backup-cannot-be-restored-automatically); keep complete private copies of both the backup and current files while choosing what to restore, and do not change saved hashes or flatten live files to force an automatic restore.
...
A failure to delete disposable skill files no longer changes a successful worker result into a failure or hides the original cancellation or timeout. [Cleanup warnings](https://docs.openclaw.ai/cli/worker#runtime-boundary) identify files that may remain; wait for the worker or session and its processes to stop before removing them manually. Credential cleanup remains separate and strict. Cloud startup also reports whether a download failed during connection, TLS, the HTTP response, or body transfer, helping operators investigate the relevant part of setup.
...
##### Install and track skill packages


The [Skills interface](https://docs.openclaw.ai/tools/skills#installer-specs) now selects the installation recipe advertised for your operating system when a skill leaves recipe IDs implicit. That corrects cases where the button showed one platform's dependency but selected another platform's recipe.
...
##### Guide dashboard building and delegated follow-through
...
##### Recover Watch companion messages
...
##### Use native realtime Talk and answer controls
...
##### Interrupt Android Talk and follow its state
...
##### Read and copy iPhone conversations
...
##### Use Android progress and dashboards
...
##### Sign in to a personal Gateway from your browser
...
##### Inspect command and pairing approvals
...
##### Reduce idle Mac background work
...
##### Select Meta Muse Spark 1.3


OpenClaw now supports [Meta's Muse Spark 1.3](https://docs.openclaw.ai/providers/meta), with Standard and Contributor choices for accounts that have access. Both accept text and image input and expose reasoning controls, a 1,048,576-token context window, and up to 131,072 output tokens. Select `meta/muse-spark-1.3` for Standard or `meta/muse-spark-1.3-contributor` for Contributor.
...
![Conceptual 2026.9.2 explainer: New conversations — Elin chooses a personal default — Existing chats stay unchanged; This conversation — Choose Account for this chat — Collaborators retain its choice. Requires an identified user and supported provider. Shared fallback may apply; this is not billing isolation.](https://github.com/user-attachments/assets/40d17386-dee0-45cd-9ea0-ed1d698fcd90)

_A personal default and a per-conversation choice are different decisions. Conceptual illustration, not a screenshot. Requires an identified user and supported provider. Shared fallback may apply; this is not billing isolation._
...
##### Find available models after refresh
...
##### Connect and discover local model services


The local-server guide now uses [llmman](https://docs.openclaw.ai/providers/llmman), the maintained replacement for Inferrs, with corrected startup commands, readiness checks, and troubleshooting for requests that work directly but fail during a larger agent task. Existing Inferrs documentation links redirect to the new guide. Keep its unauthenticated service on loopback or behind trusted access controls.
...
##### Continue and branch Codex conversations
...
##### Recover eligible provider failures
...
Ending a Gateway-controlled OpenAI Realtime call now reports a failed provider hangup and retains it for bounded automatic cleanup retries. If those attempts are exhausted, cleanup remains incomplete and the call still reserves capacity; pressing End again is not the retry mechanism. The [OpenAI cleanup guidance](https://docs.openclaw.ai/providers/openai#gateway-controlled-realtime-call-cleanup) explains recovery, including that a restart can lose the in-memory cleanup obligation and does not prove the provider call ended.
...
##### Manage existing automations from admin chat
...
##### Preserve schedules, alerts and permissions
...
##### Cancel or finish the right scheduled job
...
##### Keep background checks in their conversation
...
##### Recover delegated task results
...
##### Stop pending and running delegated work
...
##### Refresh pinned widgets in the background
...
##### Keep desktop actions on the selected machine
...
##### Explain desktop connection failures
...
##### Scroll and control Mac windows
...
##### Navigate and retire browser sessions
...
##### Keep MCP tools connected through changes
...
##### Continue a shared Beam snapshot
...
##### Keep plugin tools and schemas compatible
...
##### Keep hooks with the current session
...
##### Read directory results and locate fetched files
...
##### Preserve plugin state through cleanup
...
##### Review shared conversation access after upgrading
...
##### Keep administrative and app approvals with current authority
...
##### Keep identities and device credentials separate
...
##### Protect current conversations during reads and removal
...
##### Limit credentials and preserve structured redaction
...
##### Validate installed packages and HTTP inputs
...
##### Continue Code Mode after a tool error
...
##### Use exact paths and working guide links
...
![Conceptual 2026.9.2 explainer: Tools finish — Fictional brief edits are settled — Do not replay them; Summary interrupted — Eligible temporary provider failure — Try the summary only; Answer available — Preserve a returned CLI answer — Even if saving later fails. Not universal recovery. Cleanup errors stay visible; inspect uncertain outcomes before retrying.](https://github.com/user-attachments/assets/841c910c-8de3-4bdd-81ff-ade92a817710)

_Specific failure paths preserve already-completed work. Conceptual illustration, not a screenshot. Not universal recovery. Cleanup errors stay visible; inspect uncertain outcomes before retrying._
...
##### File reads, agent recreation, and runtime housekeeping
