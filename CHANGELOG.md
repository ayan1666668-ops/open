...
- Telegram/Reasoning display: render reasoning messages (`/reasoning on`/`stream`) as a collapsed expandable "Thinking…" blockquote — the chat stays compact and the full reasoning expands on tap; applies to sent messages, streamed previews, and preview-to-final edits.
...
- Signal/Sends: retry signal-cli RPC calls when the daemon connection cannot be established (up to 3 attempts with backoff), so sends survive daemon restarts. Timeouts and RPC errors are never retried to avoid double-sending.
- Process/Supervisor: reconcile orphaned exec children on gateway startup — managed child processes are recorded on disk and, after a gateway crash/restart, still-running orphans are killed (with PID-reuse and owner-liveness guards) instead of lingering forever.
