...
- Talk opens a dedicated conversation page with the existing animated mascot. Return to chat or press Back without ending an admitted call; **End** remains separate from agent **Stop**. Capture a still photo with **Selfie** or **Rear** camera, inspect its image preview, then use **Send photos** directly on the conversation page. Only the displayed photos are sent; unsent composer text and other attachments stay untouched. Finalized relay transcripts use the Gateway-owned chat history path. Activity follows existing Gateway events for the original conversation, including work not started by the current voice request; unavailable observation is marked incomplete. Physical background and lock-screen proof remains required.
