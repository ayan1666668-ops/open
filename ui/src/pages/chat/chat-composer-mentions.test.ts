...
it("keeps ordinary edits around selected people and everyone off the pane render path", () => {
  ...
  redraw();
  redraw.mockClear();
  edit("@everyone please review", 10);
  edit("Ask @everyone please review", 0);
  expect(redraw).not.toHaveBeenCalled();
  expect(mentions).toEqual([{ kind: "everyone", start: 4, end: 13 }]);
  expect(container.querySelector(".composer-context-strip__person-name")?.textContent).toBe(
    "Everyone with access",
  );
  edit("Ask @everyonx please review", 12, 13);
  expect(redraw).toHaveBeenCalledOnce();
  expect(mentions).toEqual([]);
});
