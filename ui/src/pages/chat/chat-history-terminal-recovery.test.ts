...
function completedHistory(runId: string): ChatHistoryResult {
  return {
    messages: [
      {
        role: "assistant",
        content: [{ type: "text", text: "The repair is complete." }],
        __openclaw: { id: "assistant-final", seq: 2, runId, runTerminal: true },
      },
    ],
    sessionInfo: {
      key: "main",
...
describe("chat history terminal recovery", () => {
  it("retires a run from its exact durable terminal when the session row omits lastRunId", async () => {
    const runId = "run-missed-terminal";
    const active = activeHistory(runId);
    const request = vi
      .fn()
      .mockResolvedValueOnce(active)
      .mockResolvedValueOnce(completedHistory(runId));
