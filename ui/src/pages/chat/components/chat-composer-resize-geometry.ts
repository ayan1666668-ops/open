...
export function clampComposerHeightPx(
  px: number,
  viewportHeight: number,
  minPx = COMPOSER_HEIGHT_MIN_PX,
): number {
  const max = Math.max(minPx, Math.floor(viewportHeight * COMPOSER_HEIGHT_MAX_VIEWPORT_RATIO));
  return Math.min(Math.max(Math.round(px), minPx), max);
}
