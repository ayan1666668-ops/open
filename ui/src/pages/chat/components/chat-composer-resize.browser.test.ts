...
it("resizes from the visible height and preserves the draft scroll position", async () => {
  await page.viewport(1440, 1000);
  const textarea = mount(true);
  ...
  vi.spyOn(grip, "setPointerCapture").mockImplementation(() => {});
  grip.dispatchEvent(new PointerEvent("pointerdown", { pointerId: 1, button: 0, clientY: 100 }));
  grip.dispatchEvent(new PointerEvent("pointermove", { pointerId: 1, clientY: 120 }));
  expect(textarea.style.maxHeight).toBe("220px");
  grip.dispatchEvent(new PointerEvent("pointerup", { pointerId: 1 }));
  textarea.value = "Draft line\n".repeat(100);
  textarea.scrollTop = 300;
  const before = textarea.scrollTop;
  grip.dispatchEvent(new PointerEvent("pointerdown", { pointerId: 2, button: 0, clientY: 100 }));
  grip.dispatchEvent(new PointerEvent("pointermove", { pointerId: 2, clientY: 120 }));
  expect(textarea.scrollTop).toBe(before);
