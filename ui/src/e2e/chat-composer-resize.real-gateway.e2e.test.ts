    ...
    async (route) => {
      ...
      await suite.withPage(
        ...
        async ({ page }) => {
          ...
          // Chat has both grips; New Session only owns width.
          await expect
            .poll(() => page.locator(TOP_GRIP).count(), { timeout: 10_000 })
            .toBe(route === "chat" ? 1 : 0);
          ...
          await expect.poll(() => readWidthSetting(page)).toBe(`${freshWidth + 160}px`);
          await page.reload();
          await waitForControlUiGatewayReady(page);
          await expect.poll(() => readColumnToken(page)).toBe(`${freshWidth + 160}px`);
          await page.locator(COMPOSER_INPUT).hover();
          ...
          await page.locator(COMPOSER_INPUT).hover();
          await dragGrip(page, SIDE_GRIP, -100, 0);
          ...
          const narrowedWidth = `${Math.max(480, shellWidthPx - 240)}px`;
          ...
          if (route === "new") {
            expect(await page.locator(TOP_GRIP).count()).toBe(0);
            expect(await page.locator(COMPOSER_TEXTAREA).evaluate((el) => el.style.maxHeight)).toBe(
              "",
            );
            return;
          }
          ...
          await expect
            .poll(async () => px(await readEditorMaxHeight(page)))
            .toBeGreaterThan(cappedHeight);
          ...
          await expect.poll(async () => px(await readEditorMaxHeight(page))).toBeLessThan(240);
          expect(
          ...
          const minimumHeight = await editor.evaluate((el) => {
