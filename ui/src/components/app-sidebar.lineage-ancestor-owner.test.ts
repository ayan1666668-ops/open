...
describe("sidebar routed-lineage freshness", () => {
  it.each(
    ...
    ].flatMap((scenario) => [true, false].map((nested) => ({ scenario, nested }))),
  )(
    "resolves a routed session without crossing its navigation boundary ($scenario, nested: $nested)",
    async ({ scenario, nested }) => {
      ...
      const child: GatewaySessionRow = {
        key: nested
          ? "agent:work:subagent:20000000-0000-4000-8000-000000000001"
          : "agent:work:dashboard:20000000-0000-4000-8000-000000000001",
        agentId: "work",
        sessionId: "work-fork-session",
        kind: "direct",
        label: nested ? "Selected Work subagent" : "Selected Work fork",
      ...
      try {
        ...
        if (nested) {
          ...
          expect(sidebar.sessionData.activeSessionLineageRoot?.key).toBe(parent.key);
        } else {
          expect(parentRow).toBeNull();
          expect(sidebar.sessionData.activeSessionLineageRoot?.key).toBe(child.key);
          expect(sidebar.sessionData.childSessionRowsByParent).toEqual({});
          expect(
            sidebar
              .querySelector(`[data-session-key="${child.key}"]`)
        ...
        expect(selection.state.selectedId).toBe("work");
        if (!nested || scenario === "owned cached parent") {
