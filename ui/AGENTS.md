...
- **Opt-in icon-grid audit:** run `pnpm lint:ui:icon-grid` manually after compact-control style changes, or during a pre-release UI review. It is not part of default Stylelint, `pnpm lint`, or an automatic release gate. The command emits JSON and exits nonzero for findings; `node scripts/audit-control-ui-icon-buttons.mts [--root <checkout>] [<css-file> ...]` is the advisory form. It joins literal Lit template ancestry with baseline stylesheet rules and shared base/component styles for pixel-sized, single-SVG, border-box controls with implicit Grid tracks and centered items. Dynamic selectors/classes, non-pixel or logical geometry, `!important`, inline styles, conditional/nested rules, cross-sheet overrides, explicit tracks, shadow/custom roots, and Flex layouts require browser proof; skipped cases are not proof of correct layout. Confirm candidates on the real page with computed styles and icon/control rectangles, preserve correct negative controls, and attach inspected before/after screenshots for any fix. Keep the known failing/fixed cases in regression coverage before expanding detection.
